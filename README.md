...
- **`Content-Type` mismatch on `PUT`** returns `415 Unsupported Media Type` on both the object server and the locator, but the check is deliberately lenient: it only rejects when the supplied `Content-Type` is one the server recognizes *and* the key's extension is not among the extensions that type maps to. A `Content-Type` the server does not recognize (e.g. `video/x-matroska` on a host whose `mime.types` only knows `video/matroska`), `application/octet-stream`, an extension-less key, and an absent `Content-Type` are all accepted. This tolerates cross-distro `/etc/mime.types` disagreements (issue #80) while still rejecting clear mismatches such as `text/plain` for a `.jpg` key.
