...
def test_scan_clean_bucket(root):
    ...
    crash_victims, garbled_lines, stale_entries = scrub.scan_bucket(root / BUCKET)
    assert crash_victims == []
    assert garbled_lines == []
    assert stale_entries == []


def test_scan_orphan_file_is_crash_victim(root):
    _write_object(root, BUCKET, "good.bin", b"committed")
    (root / BUCKET / "orphan.bin").write_bytes(b"partial")
    crash_victims, garbled_lines, stale_entries = scrub.scan_bucket(root / BUCKET)
    assert [p.name for p in crash_victims] == ["orphan.bin"]
...
def test_scan_object_with_only_garbled_entry_is_crash_victim(root):
    ...
    (root / f"{BUCKET}.sha256").write_text(f"{'a' * 70}  torn.bin\n")
    crash_victims, garbled_lines, stale_entries = scrub.scan_bucket(bucket_dir)
    assert len(garbled_lines) == 1
    assert [p.name for p in crash_victims] == ["torn.bin"]


def test_scan_stale_entry_reported(root):
    """Valid checksum line for a missing file → stale entry."""
    bucket_dir = root / BUCKET
    (root / f"{BUCKET}.sha256").write_text(f"{'a' * 64}  ghost.bin\n")
    crash_victims, garbled_lines, stale_entries = scrub.scan_bucket(bucket_dir)
    assert stale_entries == ["ghost.bin"]
    assert crash_victims == []
...
def test_dry_run_leaves_filesystem_untouched(root, capsys):
    ...
    clean = scrub.scrub_directory(root)
...
def test_delete_victims_removes_orphan(root):
    orphan = root / BUCKET / "orphan.bin"
    orphan.write_bytes(b"partial")
    clean = scrub.scrub_directory(root, delete_victims=True)
    assert clean is True
    assert not orphan.exists()


def test_delete_victims_without_repair_leaves_garbled_unhandled(root):
    cksum = root / f"{BUCKET}.sha256"
...
def test_repair_checksums_without_delete_victims(root):
    """--repair-checksums rewrites the checksum file but leaves orphan files."""
    orphan = root / BUCKET / "orphan.bin"
    orphan.write_bytes(b"partial")
    cksum = root / f"{BUCKET}.sha256"
    cksum.write_text("garbled\n")
    clean = scrub.scrub_directory(root, repair_checksums=True)
    assert clean is False   # orphan still present → unhandled
    assert orphan.exists()
...
def test_repair_strips_garbled_lines(root):
    ...
    clean = scrub.scrub_directory(root, delete_victims=True, repair_checksums=True)
    assert clean is True
...
def test_missing_directory_returns_false(tmp_path, capsys):
    clean = scrub.scrub_directory(tmp_path / "does-not-exist")
    assert clean is False
